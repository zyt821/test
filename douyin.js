...
#圈x
[rewrite local]
luckycat/aweme/v1/task/sign_in/detail? url script-request-header https://raw.githubusercontent.com/yangshuima90/GithubSync/main/DouYin/douyin.js
luckycat/aweme/v1/task/done/read? url script-request-header https://raw.githubusercontent.com/yangshuima90/GithubSync/main/DouYin/douyin.js
luckycat/aweme/v1/task/done/treasure_task? url script-request-header https://raw.githubusercontent.com/yangshuima90/GithubSync/main/DouYin/douyin.js
...
function GetCookie() {
  ...
  if ($request && $request.url.indexOf("aweme" && "done/treasure_task") >= 0) {
    const treasureheader = $request.url.split(`?`)[1]
    if (treasureheader) $.setdata(treasureheader, `treasureheader${$.idx}`)
    $.log(`[${jsname}] 获取treasure请求: 成功,treasureheader: ${treasureheader}`)
    $.msg(`获取treasureheader: 成功🎉`, ``)
    const treasurekey = JSON.stringify($request.headers)
    if (treasurekey) $.setdata(treasurekey, `treasurekey${$.idx}`)
    $.log(`[${jsname}] 获取treasure请求: 成功,treasurekey: ${treasurekey}`)
    $.msg(`获取treasurekey: 成功🎉`, ``)
  }
  if ($request && $request.url.indexOf("aweme" && "done/excitation_ad_treasure_box") >= 0) {
    const excitation_adheader = $request.url.split(`?`)[1]
    if (excitation_adheader) $.setdata(excitation_adheader, `excitation_adheader${$.idx}`)
    $.log(`[${jsname}] 获取excitation_ad请求: 成功,excitation_adheader: ${excitation_adheader}`)
    $.msg(`excitation_adheader: 成功🎉`, ``)
    const excitation_adkey = JSON.stringify($request.headers)
    if (excitation_adkey) $.setdata(excitation_adkey, `excitation_adkey${$.idx}`)
...
function treasure() {
  return new Promise((resolve, reject) => {
    ...
    $.post(watch_adurl, (error, response, data) => {
      const result = JSON.parse(data)
      if (logs) $.log(data)
      message += '📣开宝箱\n'
      if (result.err_no == 0) {
